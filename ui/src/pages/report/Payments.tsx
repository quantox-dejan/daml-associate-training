...
import {
  ...
  PaymentClaim,
  Receipt,
} from "@daml.js/nft-0.0.1/lib/Payment";
...
export default function Payments() {
  ...
  const toMe = () => [{ to: party }];
  const payableByMe = useStreamQueries<Payable, any, any>(
    Payable,
    byMe
  ).contracts;
  const payableToMe = useStreamQueries<Payable, any, any>(
    Payable,
    toMe
  ).contracts;
  const paidByMe = useStreamQueries<PaymentClaim, any, any>(
    PaymentClaim,
    byMe
  ).contracts;
  const paidToMe = useStreamQueries<PaymentClaim, any, any>(
    PaymentClaim,
    toMe
  ).contracts;
  const receiptsByMe = useStreamQueries<Receipt, any, any>(
    Receipt,
    byMe
  ).contracts;
  const receiptsToMe = useStreamQueries<Receipt, any, any>(
    Receipt,
    toMe
  ).contracts;
        ...
        <TableBody>
          {payableByMe.map((p) => (
            <TableRow key={p.contractId} className={classes.tableRow}>
              ...
              <TableCell key={1} className={classes.tableCell}>
                {formatter(p.payload.currency, p.payload.amount)}
              </TableCell>
              <TableCell key={2} className={classes.tableCell}>
                {p.payload.currency}
              </TableCell>
