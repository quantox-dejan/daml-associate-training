...
import {
  ...
  IssuerRequest,
  MintToken,
} from "@daml.js/nft-0.0.1/lib/UserAdmin";
import {
  GridList,
  GridListTile,
  GridListTileBar,
...
export default function MyWorks() {
  ...
  const tokens = useStreamQueries<Token, any, any>(Token, issuedByMe).contracts;
  const offers = useStreamQueries<TokenOffer, any, any>(
    TokenOffer,
    issuedByMe
  ).contracts;
  const issuers = useStreamQueries<Issuer, any, any>(
    Issuer,
    issuedByMe
  ).contracts;
  const myIssuerRights = issuers.length >= 1 ? issuers[0] : null;
  const issuerRequests = useStreamQueries<IssuerRequest, any, any>(
    IssuerRequest,
    issuedByMe
  ).contracts;
  ...
  type FieldsForIssuerRequest = Omit<
    Omit<IssuerRequest, "userAdmin">,
    "issuer"
  >;
  ...
  const [issuerRequestProps, setIssuerRequestProps] = useState(
    defaultIssuerRequestProps
  );
  ...
  return (
    <>
      ...
      <p>
        {myIssuerRequest
          ? "Awaiting " +
            myIssuerRequest.payload.userAdmin +
            " to Process my Request ('" +
            myIssuerRequest.payload.reason +
            "')."
          : ""}
      </p>
      <Popover
            ...
            <Table size="small">
              <TableBody>
                ...
                <TableRow className={classes.tableRow}>
                  ...
                  <TableCell key={1} className={classes.tableCell}>
                    {formatter(
                      tombstone.token.currency,
                      tombstone.token.lastPrice
                    )}
                  </TableCell>
                </TableRow>
                <TableRow className={classes.tableRow}>
                  <TableCell key={0} className={classes.tableCell}>
                    Royalty Rate per 1 {tombstone.token.currency}
                  </TableCell>
                  <TableCell key={1} className={classes.tableCell}>
                    {tombstone.token.royaltyRate}
                  </TableCell>
                </TableRow>
                <TableRow className={classes.tableRow}>
                  <TableCell key={0} className={classes.tableCell}>
